...
def load_data(dataname, datatype="numerical-for-NN", preserve_sens_in_net=0, sensitive_attributions=None,
              is_ensemble=False, kfold=5, fold_id=0, is_smaller=0):
    ...
    if not is_ensemble:
        for dataset in DATASETS:
            ...
            if dataset.dataset_name == dataname:
                ...
                sss1 = StratifiedShuffleSplit(n_splits=kfold, test_size=0.2, random_state=20230223)
                plan = sss1.split(data_x, data_label)

                for i, f in enumerate(plan):
                    if i == fold_id:
                        trainvalid_id, test_id = f
                        break

                test_org = org_data.iloc[test_id]
                test_data = data_x.loc[test_id]
                test_label = data_label.loc[test_id]

                trainvaliddata_org = org_data.iloc[trainvalid_id]
                trainvaliddata = data_x.loc[trainvalid_id]
                trainvalidlabel = data_label.iloc[trainvalid_id]
                ...
                sss2 = StratifiedShuffleSplit(n_splits=2, test_size=0.25, random_state=20230223)
                ...
                if check:
                    sen = "age_old"
                    g1 = np.sum(valid_data[sen])
                    g2 = len(valid_data[sen])-np.sum(valid_data[sen])
                    g1_1 = np.sum(valid_label.loc[np.where(valid_data[sen])])
                    g1_0 = g1 - g1_1
                    g2_1 = np.sum(valid_label.loc[np.where(1 - valid_data[sen])])
                    g2_0 = g2 - g2_1
                    print(g1_1, g1_0)
                    print(g2_1, g2_0)

                    g1 = np.sum(test_data[sen])
                    g2 = len(test_data[sen]) - np.sum(test_data[sen])
                    g1_1 = np.sum(test_label.loc[np.where(test_data[sen])])
                    g1_0 = g1 - g1_1
                    g2_1 = np.sum(test_label.loc[np.where(1 - test_data[sen])])
                    g2_0 = g2 - g2_1
                    print(g1_1, g1_0)
                    print(g2_1, g2_0)
