...
def transfer_objectives(Metric_info, Objective_t_info, obj_classes):
    obj = np.zeros([1, len(obj_classes)])
    metric = np.zeros([1, len(obj_classes)])
    for idx, obj_name in enumerate(obj_classes):
        metric[0, idx] = Metric_info[obj_name]
        if Objective_t_info[obj_name] == 0:
            obj[0, idx] = np.abs(Metric_info[obj_name])
        elif Objective_t_info[obj_name] == 1:
            if Metric_info[obj_name] == 0:
                obj[0, idx] = 1
            else:
                obj[0, idx] = 1 - np.min([Metric_info[obj_name], 1.0 / Metric_info[obj_name]])
        elif Objective_t_info[obj_name] == 2:
            obj[0, idx] = 1 - Metric_info[obj_name]
        else:
            obj[0, idx] = Metric_info[obj_name]
    return obj[0], metric[0]
...
class FairProblem(ea.Problem):  # 继承Problem父类
    def __init__(self, M=None, learning_rate=0.01, batch_size=500, sensitive_attributions=None,
                 ...
                 seed_split_traintest=2021, weight_decay=1e-3, start_time=0, is_ensemble=True, parameters=None):
        ...
        DATA, dataset_obj = load_data(dataname, preserve_sens_in_net=preserve_sens_in_net,
                                      ...
                                      kfold=self.parameters["kfold"], fold_id=self.parameters["fold_id"],
                                      is_smaller=self.parameters["is_smaller"])
        ...
        self.data_dist_train = None
        self.data_dist_valid = None
        self.data_dist_test = None
    ...
    def do_pre(self):
        ...
        # self.data_dist_train = pdist(self.train_data_norm)
        # self.data_dist_valid = pdist(self.valid_data_norm)
        # self.data_dist_test = pdist(self.test_data_norm)
        ...
        self.train_loader = DataLoader(train, batch_size=min(self.batch_size, len(train)), shuffle=True)
        self.valid_loader = DataLoader(valid, batch_size=min(self.batch_size, len(valid)), shuffle=True)
        self.test_loader = DataLoader(test, batch_size=min(self.batch_size, len(test)), shuffle=True)
        ...
        Metric_info, Objective_t_info = ea.Cal_objectives(self.train_data, self.train_data_norm,
                                                          np.array(self.train_y).reshape(1, -1),
                                                          np.array(self.train_y).reshape(1, -1),
                                                          self.sensitive_attributions,
                                                          2, obj_names=self.objectives_class,
                                                          privileged_class_names=self.privileged_class_names,
                                                          data_dist=self.data_dist_train)

        if len(self.Groups_info['group_dict_train']) > 1:
            with open('Result/' + self.dataname + '/' + self.start_time + '/detect/train_sensitive_name.txt',
                      'a+') as file:
                for sens_name in self.Groups_info['sens_idxs_train']:
                    if '+' in sens_name:
                        file.write(sens_name + ',')
                file.close()
            with open('Result/' + self.dataname + '/' + self.start_time + '/detect/valid_sensitive_name.txt',
                      'a+') as file:
            ...
            with open('Result/' + self.dataname + '/' + self.start_time + '/detect/test_sensitive_name.txt',
                      'a+') as file:
            ...
            for sens_name in self.Groups_info['sens_idxs_train']:
                if '+' in sens_name:
                    ...
                    idx_count += 1
            np.savetxt('Result/' + self.dataname + '/' + self.start_time + '/detect/train_idxs_sensitive.txt',
                       idx_sens_train)
            ...
            for sens_name in self.Groups_info['sens_idxs_valid']:
                if '+' in sens_name:
                    ...
                    idx_count += 1
            np.savetxt('Result/' + self.dataname + '/' + self.start_time + '/detect/valid_idxs_sensitive.txt',
                       idx_sens_valid)
            ...
            for sens_name in self.Groups_info['sens_idxs_test']:
                if '+' in sens_name:
                    ...
                    idx_count += 1
            np.savetxt('Result/' + self.dataname + '/' + self.start_time + '/detect/test_idxs_sensitive.txt',
                       idx_sens_test)

            if self.is_ensemble:
                with open('Result/' + self.dataname + '/' + self.start_time + '/detect/ensemble_sensitive_name.txt',
                          'a+') as file:
                ...
                for sens_name in self.Groups_info['sens_idxs_ensemble']:
                    if '+' in sens_name:
                        ...
                        idx_count += 1
                np.savetxt('Result/' + self.dataname + '/' + self.start_time + '/detect/ensemble_idxs_sensitive.txt',
                           idx_sens_ensemble)

        else:

            with open(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/train_sensitive_name.txt',
                      'a+') as file:
                for sens_name in self.Groups_info['sens_idxs_train']:
                    file.write(sens_name + ',')
                file.close()
            with open(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/valid_sensitive_name.txt',
                      'a+') as file:
                for sens_name in self.Groups_info['sens_idxs_valid']:
                    file.write(sens_name + ',')
                file.close()

            with open(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/test_sensitive_name.txt',
                      'a+') as file:
                for sens_name in self.Groups_info['sens_idxs_test']:
                    file.write(sens_name + ',')
                file.close()
            ...
            for sens_name in self.Groups_info['sens_idxs_train']:
                idx_sens_train[0][self.Groups_info['sens_idxs_train'][sens_name][0]] = idx_count
                idx_count += 1
            np.savetxt(self.parameters["dirName"] + '/'+ self.dataname + '/' + self.start_time + '/detect/train_idxs_sensitive.txt',
                       idx_sens_train)
            ...
            for sens_name in self.Groups_info['sens_idxs_valid']:
                idx_sens_valid[0][self.Groups_info['sens_idxs_valid'][sens_name][0]] = idx_count
                idx_count += 1
            np.savetxt(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/valid_idxs_sensitive.txt',
                       idx_sens_valid)
            ...
            for sens_name in self.Groups_info['sens_idxs_test']:
                idx_sens_test[0][self.Groups_info['sens_idxs_test'][sens_name][0]] = idx_count
                idx_count += 1
            np.savetxt(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/test_idxs_sensitive.txt',
                       idx_sens_test)

            if self.is_ensemble:
                with open(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/ensemble_sensitive_name.txt',
                          'a+') as file:
                    for sens_name in self.Groups_info['sens_idxs_ensemble']:
                        file.write(sens_name + ',')
                    file.close()
                ...
                for sens_name in self.Groups_info['sens_idxs_ensemble']:
                    idx_sens_ensemble[0][self.Groups_info['sens_idxs_ensemble'][sens_name][0]] = idx_count
                    idx_count += 1
                np.savetxt(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/ensemble_idxs_sensitive.txt',
                           idx_sens_ensemble)
                np.savetxt(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/ensemble_truelabel.txt',
                           np.array(self.ensemble_y))

        np.savetxt(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/train_truelabel.txt',
                   np.array(self.train_y))
        np.savetxt(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/valid_truelabel.txt',
                   np.array(self.valid_y))
        np.savetxt(self.parameters["dirName"] + '/' + self.dataname + '/' + self.start_time + '/detect/test_truelabel.txt',
    ...
    def aimFunc(self, pop, gen=0, dirName=None, loss_type=-1):  # 目标函数
        ...
        Allmetric_valid = np.zeros([popsize, len(self.objectives_class)])
        Allmetric_test = np.zeros([popsize, len(self.objectives_class)])
        Allmetric_train = np.zeros([popsize, len(self.objectives_class)])
        Allmetric_ensemble = np.zeros([popsize, len(self.objectives_class)])
        for idx in range(popsize):
            ...
            if loss_type != -1:
                ...
                for epoch in range(self.epoches):
                    ...
                    # for i, (x_batch, y_batch, s_batch, data_idx) in enumerate(self.train_loader):
                    if self.use_gpu:
                        x_batch, y_batch = self.x_batch.cuda(), self.y_batch.cuda()
                    else:
                        x_batch, y_batch = self.x_batch, self.y_batch
                    y_logits, y_pred = individual(x_batch)

                    loss_list = []
                    if "BCE_loss" in self.objectives_class or "accuracy" in self.objectives_class:
                        loss_list.append(loss_fn(y_logits, y_batch))

                    # losses_ = calcul_all_fairness(self.train_data.loc[self.data_idx.detach()], y_pred,
                    #                               y_batch, self.sensitive_attributions, 2)
            ...
            with torch.no_grad():
                ...
                logits_test = pred_sigmoid_temp.detach().numpy()
                ...
                Metric_info_test, Objective_t_info_test = ea.Cal_objectives(self.test_data,
                                                                            self.test_data_norm,
                                                                            logits_test, self.y_test,
                                                                            self.sensitive_attributions,
                                                                            2, obj_names=self.objectives_class,
                                                                            privileged_class_names=self.privileged_class_names,
                                                                            data_dist=self.data_dist_test)

                # in the train data
                logit_temp, pred_sigmoid_temp = individual(self.x_train)
                logits_train = pred_sigmoid_temp.detach().numpy()
                pred_label_train[idx][:] = get_label(logits_train.reshape(1, -1).copy())
                pred_logits_train[idx][:] = logits_train.reshape(1, -1)
                Metric_info_train, Objective_t_info_train = ea.Cal_objectives(self.train_data,
                ...
                logits_valid = pred_sigmoid_temp.detach().numpy()
                pred_label_valid[idx][:] = get_label(logits_valid.reshape(1, -1).copy())
                pred_logits_valid[idx][:] = logits_valid.reshape(1, -1)
                Metric_info_valid, Objective_t_info_valid = ea.Cal_objectives(self.valid_data,
                                                                              self.valid_data_norm,
                                                                              logits_valid, self.y_valid,
                                                                              self.sensitive_attributions,
                                                                              2, obj_names=self.objectives_class,
                                                                              privileged_class_names=self.privileged_class_names,
                                                                              data_dist=self.data_dist_valid)

                if self.is_ensemble:
                    # in the ensemble data
                    logit_temp, pred_sigmoid_temp = individual(self.x_ensemble)
                    logits_ensemble = pred_sigmoid_temp.detach().numpy()
                    pred_label_ensemble[idx][:] = get_label(logits_ensemble.reshape(1, -1).copy())
                    pred_logits_ensemble[idx][:] = logits_ensemble.reshape(1, -1)
                    Metric_info, Objective_t_info = ea.Cal_objectives(self.ensemble_data,
                                                                      self.ensemble_data_norm,
                                                                      logits_ensemble, self.y_ensemble,
                                                                      self.sensitive_attributions,
                                                                      2, obj_names=self.objectives_class)

                AllObj_train[idx][:], Allmetric_train[idx][:] = \
                    transfer_objectives(Metric_info_train, Objective_t_info_train, self.objectives_class)
                AllObj_test[idx][:], Allmetric_test[idx][:]  = \
                    transfer_objectives(Metric_info_test, Objective_t_info_test, self.objectives_class)
                AllObj_valid[idx][:], Allmetric_valid[idx][:]  = \
                    transfer_objectives(Metric_info_valid, Objective_t_info_valid, self.objectives_class)
                if self.is_ensemble:
                    AllObj_ensemble[idx][:] = get_obj_vals(Groups_info_ensemble, self.objectives_class)
        ...
        pop.pred_logits_train = Allmetric_train
        pop.pred_logits_valid = Allmetric_valid
        pop.pred_logits_test = Allmetric_test
        pop.pred_logits_ensemble = pred_logits_ensemble if self.is_ensemble else None
